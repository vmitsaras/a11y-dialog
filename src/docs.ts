...
import { A11Y_DIALOG_ASYNC_ACTION_EVENTS } from "./async-action";
import { A11Y_DIALOG_OUTCOME_EVENTS } from "./outcome";
  ...
  events: [
    {
      ...
      detail: ["instance", "dialog", "trigger", "open"],
      target: "dialog",
      bubbles: true,
      composed: false,
      cancelable: false
    ...
    ...[
      [A11Y_DIALOG_EVENTS.beforeOpen, "Before an effective open; preventDefault keeps the dialog closed.", true],
      [A11Y_DIALOG_EVENTS.open, "After open state and initial focus are applied.", false],
      [A11Y_DIALOG_EVENTS.beforeClose, "Before an effective core close; preventDefault keeps the dialog open.", true],
      [A11Y_DIALOG_EVENTS.close, "After close state and focus restoration are applied.", false],
      [A11Y_DIALOG_EVENTS.change, "After an effective open or close transition.", false],
      [A11Y_DIALOG_EVENTS.destroy, "After core cleanup finishes.", false]
    ...
    })),
    {
      ...
      detail: ["instance", "dialog", "statusTarget", "outcome", "source", "reason", "message"],
      target: "dialog",
      bubbles: true,
      composed: false,
      cancelable: false
